...
class Student:
    def __init__ (self, name, id, key):
        ...
        if (isValidID(id)):
            ...
            print("INVALID ID - SETTING DEFAULT ID NUMBER: " + str(key))
            self.id = key
        ...
        for i in range(0, len(self.enrolledCourses)):
            if (self.enrolledCourses[i].name.lower() == course.lower()):
                ...
                if (isValidUnits(grade)):
                    self.grades[self.enrolledCourses[i].name] = grade
...
def AddStudent ():
    ...
    global idKey
    AllStudents.append(Student(name, id, idKey))
    idKey = idKey + 1
...
def TraverseGrades (student):
    inc = lambda x, y: x + y
    calcGPA = lambda grade, units: 1.0 * grade / units
    tUnits = 0
    tGrade = 0
    for i in range(0, len(student.enrolledCourses)):
...
def displayEnrollmentMenu():
    ...
    # view student's report card
        ...
        s_exist = False
        for i in range(0, len(AllStudents)):
            if (AllStudents[i].name.lower() == sName.lower()):
    ...
    # give student grade in certain course   
        ...
        choice_g = float(input("SET GRADE TO: "))
        print()
